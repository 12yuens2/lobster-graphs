...
def contains(nodes, node):
    for n in nodes:
        if n.label == node.label and n.kp == node.kp:
            return True

    return False
    
def create_graph(subgraphs: List[Tuple[KeyLabel, ...]]) -> Graph:
    nodes: List[Node] = []
    edges: List[Edge] = []
    for permutation in subgraphs:
        subgraph = graph_from_permutation(permutation)
        for n in subgraph.nodes:
            if not contains(nodes, n):
                nodes.append(n)
