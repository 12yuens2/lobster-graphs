...
** Week 2
*** Work:
1. Extract all filtered keypoints
2. Build .gdf file with filtered keypoints (width, height, x, y)
3. Build combinations of node connections for graph matching

Build hand annotated graphs and run all keypoints into graph matching

Steps:
1. Get keypoints with opencv and filter as much as possible
2. Get combinations of triplet keypoints and make them node/edges with all combinations of possible labels
3a, Filter by removing combinations with incompatible sizes/edge lengths
3b, Filter by removing combinations with incompatible labels
