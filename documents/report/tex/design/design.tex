...
\end{figure}
\noindent
A subset of the dataset was taken to create a database of manually annotated attributed lobster graphs. This database is important for many aspects of the overall method as it is the set of complete graphs for subgraph matching. Further, the attributes such as node labels, node sizes and edge weights contribute as a basis of the probability models used in both subgraph creation and rebuilding a complete lobster graph. 
...
With a set of keypoints extracted and filtered from the image, the next step was to label the keypoints and create combinations and permutations of subgraphs to be matched.
...
\end{equation}
The probability of $P(size | label)$ can be found using a normal distribution for all sizes of a label found in the annotated dataset. With the probability for each possible labelling for every keypoint, the labels can be applied based on a probability threshold. This means multiple labels can be applied to the same keypoint and each separate labelling must be treated as a separate node during subgraph creation. The threshold for labelling represents another trade off between computation required and completeness. If the threshold is too high, then we may miss many potential labels, but if the threshold is too low, then too many labels are applied, leading to a combinatorial explosion when creating the permutations of subgraphs. The result of the labelling process is a list of tuples containing a keypoint and its label. The same keypoint can appear multiple times with different labels.
...
\centering
	...
	\end{subfigure}
\caption{The order of the permutations matter as it determines the edges between the nodes.}
\end{figure}
\noindent
