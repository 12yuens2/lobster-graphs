...
\end{figure}
In this project, the problem of graph matching extends beyond that of pure subgraph isomorphism, where only the number of vertices and its connections are relevant, though the normal subgraph isomorphism problem is also applied as part of the method pipeline. The extension comes from the fact that the shape of the lobster is a crucial part in determining successful matches. As was shown in section \ref{sec:cattle}, the size (weight) of each node in the graph and the length (weight) of the edges are all included important aspects in matching and are included during the final matching step. 